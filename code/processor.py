...
def uuid_from_text(text):
    return uuid.uuid5(namespace=null_uuid, name=text).hex
...
parser.add_argument(
    "-p",
    type=str,
    default="eds_data/data_sources/research_papers_pdf",
    help="Provide name of the input directory where pdf files to process are saved",
)
parser.add_argument(
    "-x",
    type=str,
    default="eds_data/data_sources/research_papers_xml",
    help="Provide name of the input directory where xml files to process are saved",
)
parser.add_argument(
    "-o",
    type=str,
    default="eds_data/training_data/json_files",
    help="Provide name of the output directory to save the output JSON file",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="if format is pdf, which page number to consider for creating questions",
)
parser.add_argument(
    "-q",
    type=int,
    default=2,
    help="if format is pdf, how many questions per page should be created",
)
...
class XMLVectorProcessor(DirectoryProcessor):
    def __init__(
        self,
        input_directory_path="eds_data/data_sources/research_papers_xml",
        output_directory_path="eds_data/vector_data/json_files",
        batch_size=20,
        batch_delay_sec=None,
    ):
        super().__init__(
    ...
    def _process_one(self, file_path):
        try:
            ...
            doi = tree.find(".//*[@pub-id-type='doi']").text

            body_node = tree.find(".//body")
            body_texts = [t for t in body_node.itertext()]
            body = " ".join(body_texts)

            text_splitter = RecursiveCharacterTextSplitter.from_tiktoken_encoder(
                encoding_name="cl100k_base", chunk_size=512, chunk_overlap=128
            )
            ...
            print(f"Pushed {len(vectors)} vectors.")
...
def prune_files():
    ...
    if len(files_corrupted) > 0:
        print(
            f"{len(files_corrupted)} files have corrupted JSON entries. Hence, removing them from the output directory ..."
        )
...
def main():
    ...
    # XMLVectorProcessor(batch_size=1, batch_delay_sec=100).run()
