from utility import REPO_ROOT_PATH, ParseException, create_question_from_file_v2
...
parser.add_argument('-i', type=str, default="eds_data/data_sources/research_papers_pdf",  help='Provide name of the input directory where files to process are saved')
...
parser.add_argument('-p', type=int, default=0, help='if format is pdf, which page number to consider for creating questions')
    ...
    def run(self):
        ...
        for i in print_progress(range(0, len(unprocessed_file_paths), self.batch_size)):
            ...
            if self.batch_delay_sec:
                print(f"Sleeping {self.batch_delay_sec} seconds to avoid API rate limits...")
                time.sleep(self.batch_delay_sec)

    def _process_batch(self, filename_batch):
        """
...
total_process_exceptions = 0
class PDFDirectoryProcessor(DirectoryProcessor):
    def _process_batch(self, path_batch):

        print("Starting batch...")

        with concurrent.futures.ThreadPoolExecutor(max_workers=len(path_batch)) as executor:
            futures = set(executor.submit(self._process_pdf, file_path) for file_path in path_batch)

    def _process_pdf(self, file_path):
        try:
            training_data = create_question_from_file_v2(
                    file_path, page_index=args.p, number_of_questions_per_page=args.q
            )
        ...
        file_name = Path(file_path.replace(self.input_extension, self.output_extension)).name
        output_path = self.output_directory_path / file_name

        with open(output_path, "w") as json_file:
            json.dump(training_data, json_file, indent=4, sort_keys=True)
