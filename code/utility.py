...
REPO_ROOT_PATH = (Path(__file__) / ".." / ".." ).resolve()
...
def remove_escape_and_control_characters(input_string):
    # Use regular expression to remove escape characters and control characters
    return re.sub(r'[\x00-\x1F\x7F\\]', '', input_string)

def get_page_text_mutexed(file, page_index):
    """
    fitz.open is unforuntaely not thread-safe, so we're 
    only allowing one thread to use it at a time
    """
    mutex.acquire()
    ...
    try:
        ...
        for page in doc[page_index:]:
            page_text = page.get_text()

            if page_text:
                return remove_escape_and_control_characters(page_text)
        
        return ''
    except Exception as e:
        print(e)
        raise ParseException(f"Couldn't open {file}")

    finally:
        # Release the mutex after accessing the critical section
        if doc:
            doc.close()
...
def create_question_from_file(file, llm, page_index=0, number_of_questions_per_page=2):
    ...
    page_text = get_page_text_mutexed(file, page_index)
...
def create_question_from_text_v2(text, number_of_questions=2):
    ...
    text = 'Here is the input text you have been asked to generate Q&A for:\n\n' + text
